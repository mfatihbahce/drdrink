...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
        <h1 class="text-xl font-bold text-gray-900">Kullanıcı Düzenle: {{ $user->name }}</h1>
        <p class="text-sm text-gray-500 mt-1">Kullanıcı bilgilerini ve mağaza atamalarını güncelleyin.</p>
    </div>
    <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-6">
        ...
        <div class="space-y-6">
            ...
            <div>
                <label for="password" class="block font-medium text-gray-700 mb-1">Yeni Şifre (boş bırakılırsa değişmez)</label>
            ...
            <div>
                ...
                <select name="role" id="role" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                ...
                <div class="space-y-2 max-h-40 overflow-y-auto border border-gray-200 rounded-lg p-3">
                    @foreach($stores as $store)
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="store_ids[]" value="{{ $store->id }}" {{ in_array($store->id, old('store_ids', $user->stores->pluck('id')->toArray())) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-600 focus:ring-amber-500">
