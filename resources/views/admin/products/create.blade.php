    ...
    <form action="{{ route('admin.products.store') }}" method="POST" class="p-6">
        ...
        <div class="grid gap-6 sm:grid-cols-2">
            ...
            <div>
                ...
                <select name="store_id" id="store_id" required class="w-full border border-gray-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                    @foreach($stores as $s)
                        <option value="{{ $s->id }}" {{ old('store_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->city->name }})</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="category_id" required class="w-full border border-gray-200 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" data-store="{{ $cat->store_id }}" {{ old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }} ({{ $cat->store?->name }})</option>
                    @endforeach
                </select>
            </div>
            <div>
