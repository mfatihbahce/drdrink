...
<div class="w-full">
    ...
    <h1 class="font-display text-2xl sm:text-3xl font-light text-gray-900 mb-8">Adres Düzenle</h1>

    <form action="{{ route('addresses.update', $address) }}" method="POST" class="bg-white rounded-2xl border border-gray-200 p-6 sm:p-8 space-y-5">
        @csrf
        @method('PATCH')

        <div>
            <label class="block text-sm font-medium text-gray-600 mb-1.5">Adres Başlığı</label>
            <input type="text" name="title" value="{{ old('title', $address->title) }}" required placeholder="Ev, İş, vb." class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <input type="text" name="district" value="{{ old('district', $address->district) }}" required placeholder="Örn: Merkez" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="neighborhood" value="{{ old('neighborhood', $address->neighborhood) }}" required placeholder="Mahalle adı" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
            ...
            <div>
                ...
                <input type="text" name="street" value="{{ old('street', $address->street) }}" required placeholder="Sokak adı" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            <div>
                ...
                <input type="text" name="avenue" value="{{ old('avenue', $address->avenue) }}" placeholder="Cadde adı" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
            ...
            <div>
                ...
                <input type="text" name="building" value="{{ old('building', $address->building) }}" required placeholder="Bina numarası" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
            ...
            <div>
                ...
                <input type="text" name="apartment" value="{{ old('apartment', $address->apartment) }}" placeholder="Daire no / Kapı no" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none">
        ...
        <div class="pt-2 sm:pt-4 border-t border-gray-100 flex items-center">
            <input type="checkbox" name="is_default" id="is_default" value="1" {{ old('is_default', $address->is_default) ? 'checked' : '' }} class="rounded border-gray-300 text-amber-500 focus:ring-amber-500">
            <label for="is_default" class="ml-2 text-sm text-gray-600">Varsayılan adres olarak ayarla</label>
        </div>

        <div class="pt-4 flex gap-3">
            <button type="submit" class="bg-amber-500 text-black px-6 py-3 rounded-lg font-medium hover:bg-amber-400 transition">Güncelle</button>
