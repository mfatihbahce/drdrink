...
@section('content')
<div class="mb-6">
    <a href="{{ route('store.categories.index') }}" class="text-amber-600 hover:text-amber-700 font-medium">← Kategorilere Dön</a>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
        <h1 class="text-xl font-bold text-gray-900">Yeni Kategori Ekle</h1>
        <p class="text-sm text-gray-500 mt-1">Ürün gruplandırması için yeni kategori ekleyin.</p>
    </div>
    <form action="{{ route('store.categories.store') }}" method="POST" class="p-6">
        @csrf
