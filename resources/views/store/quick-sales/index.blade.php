...
<div class="pos-orders-page" style="display: flex; flex-direction: column; min-height: 100vh;"
     data-orders-poll data-last-order-id="{{ $orders->isNotEmpty() ? $orders->first()->id : 0 }}" data-last-order-at="{{ $orders->isNotEmpty() ? $orders->first()->created_at->toIso8601String() : now()->toIso8601String() }}" data-poll-url="{{ route('store.quick-sales.index') }}" data-poll-q="{{ request('q') }}" data-poll-status="{{ request('status') }}">
    <header class="pos-orders-header">
        <div style="display: flex; align-items: center; gap: 20px;">
            <a href="{{ route('store.dashboard') }}" style="display: flex; align-items: center; gap: 12px; text-decoration: none; color: inherit;">
                <div class="pos-logo">☕</div>
                <h1 style="font-size: 1.25rem; font-weight: 700; margin: 0;">DrDrink</h1>
            </a>
            <div class="pos-mode-tabs">
                <a href="{{ route('store.pos.index') }}" class="pos-mode-tab">Satış Ekranı</a>
                <span class="pos-mode-tab active">Hızlı Satışlar</span>
        ...
        <div style="display: flex; align-items: center; gap: 16px; font-size: 0.875rem; color: var(--pos-text-muted);">
            <a href="{{ route('store.dashboard') }}" class="pos-link">{{ $store->name ?? '' }}</a>
    ...
    <div id="new-order-alert-banner" class="hidden" style="margin: 16px 24px 0; padding: 16px 20px; background: rgba(34, 197, 94, 0.15); border: 1px solid rgba(34, 197, 94, 0.3); border-radius: 12px; display: flex; align-items: center; gap: 12px; color: #22c55e;">
        ...
        <span class="font-semibold">Yeni hızlı satış eklendi! Liste güncellendi.</span>
    </div>
    ...
    <div class="pos-orders-content">
        <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 20px;">
            <h2 style="font-size: 1.5rem; font-weight: 700; margin: 0;">Hızlı Satışlar</h2>
            <form action="{{ route('store.quick-sales.index') }}" method="GET" style="display: flex; gap: 10px;">
            ...
            <div style="overflow-x: auto;">
                <table class="pos-orders-table">
                    ...
                    <tbody id="orders-tbody">
                        @include('store.quick-sales._table')
                    </tbody>
                </table>
            </div>
...
(function() {
    ...
    var skipFirst = true;
    function poll() {
        var base = pollEl.dataset.pollUrl || '{{ route("store.quick-sales.index") }}';
